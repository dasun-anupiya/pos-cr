...
const TaxReports: React.FC = () => {
  ...
  // Move taxData inside component to use translation function
  const taxData = {
    monthly: {
      totalSales: 85000,
      totalTax: 15300,
      details: [
        { category: t('reports:tax.salesTax'), taxable: 85000, rate: 0.18, amount: 15300 },
        { category: t('reports:tax.vat'), taxable: 0, rate: 0, amount: 0 },
        { category: t('reports:tax.serviceTax'), taxable: 0, rate: 0, amount: 0 },
      ],
      summary: {
        totalTaxableAmount: 85000,
        totalTaxAmount: 15300,
      },
    },
  };
        ...
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Box sx={{ mb: 4 }}>
            <Typography variant="h6" gutterBottom>
              {t('reports:tax.details')}
            </Typography>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '1rem' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #e0e0e0' }}>
                    <th style={{ padding: '12px 16px', textAlign: 'left' }}>{t('reports:tax.category')}</th>
                    <th style={{ padding: '12px 16px', textAlign: 'right' }}>{t('reports:tax.taxableAmount')}</th>
                    <th style={{ padding: '12px 16px', textAlign: 'right' }}>{t('reports:tax.rate')}</th>
                    <th style={{ padding: '12px 16px', textAlign: 'right' }}>{t('reports:tax.amount')}</th>
                  </tr>
                </thead>
                <tbody>
                  {taxData.monthly.details.map((item) => (
                    <tr key={item.category} style={{ borderBottom: '1px solid #e0e0e0' }}>
                  ...
                  <tr style={{ borderTop: '2px solid #e0e0e0', backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>
                    <td style={{ padding: '12px 16px' }}>{t('reports:tax.total')}</td>
                    <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                      ${taxData.monthly.summary.totalTaxableAmount.toLocaleString()}
                    </td>
                    <td style={{ padding: '12px 16px', textAlign: 'right' }}></td>
                    <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                      ${taxData.monthly.summary.totalTaxAmount.toLocaleString()}
                    </td>
                  </tr>
                </tbody>
              </table>
